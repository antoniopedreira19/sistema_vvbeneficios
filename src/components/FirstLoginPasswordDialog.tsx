...
const FirstLoginPasswordDialog = ({ open, onPasswordChanged }: FirstLoginPasswordDialogProps) => {
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Buscar o empresa_id do profile para atualizar o email_contato da empresa
      ...
      // Se o usuário está vinculado a uma empresa, atualizar o email_contato
      if (profileData?.empresa_id) {
        const { error: empresaError } = await supabase
          .from("empresas")
          .update({ email_contato: email.trim() })
          .eq("id", profileData.empresa_id);

        if (empresaError) {
          console.error("Erro ao atualizar email da empresa:", empresaError);
        }
      }
